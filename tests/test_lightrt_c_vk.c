...
/* Hardware ray_query: build an AS on the GPU and trace it; compare to a CPU
 * Moller-Trumbore trace (lrt_tri_intersect1) over the same triangles. */
static void test_path_rtx(lrt_vk_engine *e, const float *v, size_t ntris,
    ...
    int hits = lrt_vk_trace_scene_rtx(e, v, (uint32_t)ntris, rays,
                                      (uint32_t)nrays, gh, &err);
    CHECK(hits >= 0, "[%s] ray_query trace failed: %s", label,
          lrt_vk_engine_last_error(e));
    if (hits < 0) {
        free(rays);
        free(gh);
    ...
    CHECK(mism == 0, "[%s] ray_query: %d/%zu rays mismatch HW vs CPU", label, mism,
          nrays);
    printf("  [%s] ray_query ok: %d mismatches over %zu rays (%d HW hits)\n", label,
           mism, nrays, hits);
    free(rays);
    free(gh);
    lrt_tri_scene_free(s);
